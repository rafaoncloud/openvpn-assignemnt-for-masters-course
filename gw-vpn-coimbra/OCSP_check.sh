...
# OCSP Check Script for OpenVPN
# Retuns 0 if the status is good otherwise return 1 and fails to authenticate the user.

# Changed from url to host

# Example of command produced:
# openssl ocsp -CAfile cacert.pem -issuer cacert.pem -host 192.168.0.253:81 -serial 2

# Output
#Response verify OK
#2: good
#	This Update: Apr 10 19:49:00 2019 GMT

#openssl ocsp -CAfile cacert.pem -issuer cacert.pem -host 192.168.0.253:81 -serial 4

# Output
#Response verify OK
#4: unknown
#	This Update: Apr 10 19:52:56 2019 GMT
...
if [ $check_depth -eq -1 ] || [ $cur_depth -eq $check_depth ]; then
  ...
  if [ -n "$serial" ]; then
    ...
    # SWITCHER -url to -host
    # -url "$ocsp_url" \

    echo openssl ocsp -issuer "$issuer" \
                    "$nonce" \
                    -CAfile "$verify" \
		     -host "$ocsp_url" \
                    -serial "${serial}"
